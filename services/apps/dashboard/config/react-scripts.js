...
switch(process.argv[2]) {
  ...
  case 'start':
    rewireModule('react-scripts/scripts/start.js', loadCustomizer('./webpack.development.config'));
    break;
  // The "build" script is run to produce a production bundle
  case 'build':
    rewireModule('react-scripts/scripts/build.js', loadCustomizer('./webpack.production.config'));
    break;
  ...
  case 'test':
    ...
    let customizer = loadCustomizer('./webpack.testing.config');
